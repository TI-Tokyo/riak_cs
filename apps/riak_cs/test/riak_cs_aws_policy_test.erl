...
-module(riak_cs_aws_policy_test).
...
parse_ip_test_()->
    [
     ?_assertEqual({{192,0,0,1}, {255,0,0,0}},
                   riak_cs_aws_policy:parse_ip(<<"192.0.0.1/8">>)),
     ?_assertEqual({error, einval},
                   riak_cs_aws_policy:parse_ip(<<"192.3.1/16">>)),
     ?_assertEqual(<<"1.2.3.4">>,
                   riak_cs_aws_policy:print_ip(riak_cs_aws_policy:parse_ip(<<"1.2.3.4">>))),
     ?_assertEqual(<<"1.2.3.4/13">>,
                   riak_cs_aws_policy:print_ip(riak_cs_aws_policy:parse_ip(<<"1.2.3.4/13">>))),
     ?_assertEqual({error, einval},
                   riak_cs_aws_policy:parse_ip(<<"0">>)),
     ?_assertEqual({error, einval},
                   riak_cs_aws_policy:parse_ip(<<"0/0">>))
    ].

empty_statement_conversion_test() ->
    Policy = ?POLICY{id = <<"hello">>, statement = [#statement{}]},
    {error, missing_principal} = riak_cs_aws_policy:policy_from_json(
...
sample_policy_check_test() ->
    ...
    Access = #access_v1{method = 'GET',
                        target = object,
                        id = <<"spam/ham/egg">>,
                        req = #wm_reqdata{peer = "192.168.0.1"},
...
my_split_test_()->
    [
     ?_assertEqual(["foo", "bar"], riak_cs_aws_policy:my_split($:, "foo:bar", [], [])),
     ?_assertEqual(["foo", "", "", "bar"], riak_cs_aws_policy:my_split($:, "foo:::bar", [], [])),
     ?_assertEqual(["arn", "aws", "s3", "", "", "hoge"],
                   riak_cs_aws_policy:my_split($:, "arn:aws:s3:::hoge", [], [])),
     ?_assertEqual(["arn", "aws", "s3", "", "", "hoge/*"],
                   riak_cs_aws_policy:my_split($:, "arn:aws:s3:::hoge/*", [], []))
    ].
...
parse_arn_test()->
    ...
    {ok, ARNS0} = riak_cs_aws_policy:parse_arns(List0),
    ?assertEqual(List0, riak_cs_aws_policy:print_arns(ARNS0)),

    List1 = [<<"arn:aws:s3:ap-northeast-1:000000:hoge">>, <<"arn:aws:s3:::hoge/*">>],
    {ok, ARNS1} = riak_cs_aws_policy:parse_arns(List1),
    ?assertEqual(List1, riak_cs_aws_policy:print_arns(ARNS1)),

    ?assertEqual({error, bad_arn}, riak_cs_aws_policy:parse_arns([<<"asdfiua;sfkjsd">>])),
...
malformed_policy_json_test()->
    ...
    {error, malformed_policy_json} = riak_cs_aws_policy:policy_from_json(JsonPolicy0).
